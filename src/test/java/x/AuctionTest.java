...
class AuctionTest {
    @Test
    void testClone() {
        Auction auction = new Auction(5, LocalDateTime.parse("2018-01-01T12:00"), new User("Joe"), new Product("computer"));
        ...
        assertEquals(5, another.getPrice());
        assertEquals(LocalDateTime.parse("2018-01-01T12:00"), another.getStart());
        assertEquals("Jack", another.getUser().getName()); //Shallow copy miatt
        assertEquals("computer", another.getProduct().getName());
    }
